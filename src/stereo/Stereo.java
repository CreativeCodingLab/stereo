...
public class Stereo {
    ...
    public void setRightEyeView(PGL pgl) {
        ...
        if(this.stereoType == StereoType.PASSIVE) {
            pgl.viewport(this.w / 2, 0, this.w / 2, this.h);
        } else {
            pgl.viewport(0, 0, this.w, this.h);
        }

        // Set frustum
        float top = (float)(widthdiv2);
        float bottom = (float)(-widthdiv2);
        float left = (float)(-aspectRatio * widthdiv2 - 0.5 * eyeSeperation * nearPlane / convPlane);
        float right = (float)(aspectRatio * widthdiv2 - 0.5 * eyeSeperation * nearPlane / convPlane);
    ...
    public void setLeftEyeView(PGL pgl) {
        // Adjusts viewport based on stereo type
        if(this.stereoType == StereoType.PASSIVE) {
            pgl.viewport(0, 0, this.w / 2, this.h);
        } else {
            pgl.viewport(0, 0, this.w, this.h);
        }
